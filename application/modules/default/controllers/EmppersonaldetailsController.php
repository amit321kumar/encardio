		 			...
		 			else
		 			{
		 				...
		 				if(!empty($empdata))
		 				{
		 					...
		 					if(isset($data[0]['ethniccodeid']) && $data[0]['ethniccodeid'] !='')
                                ...
                                if($data[0]["doa"] !='')
								{
		 						$doa = sapp_Global::change_date($data[0]["doa"], 'view');
		 						$emppersonaldetailsform->doa->setValue($doa);
								}
		 						/*
		...
		if($auth->hasIdentity()){
			$loginUserId = $auth->getStorage()->read()->id;
		        ...
		        if(!empty($new_post_values))
		        {
					...
					$doa = $this->_request->getParam('doa');
					$doa = sapp_Global::change_date($doa, 'database');
