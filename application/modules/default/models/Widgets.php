...
class Default_Model_Widgets extends Zend_Db_Table_Abstract
{
	...
	public function getTodaysAniversaries($businessunit_id,$department_id,$isOrganizationHead='')
	{
		$db = Zend_Db_Table::getDefaultAdapter();
		$whereStr = '';
		if($isOrganizationHead != 1)
		...
		$whereStr .= " and e.isactive = 1";
		$qryStr = "select id,firstname, lastname from main_users where id in (Select p.user_id from main_emppersonaldetails p inner join main_employees e on p.user_id = e.user_id  where day(now()) = day(p.doa) and month(now()) = month(p.doa) ".$whereStr.")";
		...
		public function getUpcomingAniversaries($businessunit_id,$department_id,$isOrganizationHead='')
	{
		$db = Zend_Db_Table::getDefaultAdapter();
		$whereStr = '';
		if($isOrganizationHead != 1)
		...
		* @var String $qryStr
		*/
		$qryStr = 'SELECT e.userfullname, p.doa FROM  main_emppersonaldetails p  
					inner join main_employees_summary e on p.user_id = e.user_id 
					where  DATE_ADD(p.doa, 
                	INTERVAL YEAR(CURDATE())-YEAR(p.doa)
                         + IF(DAYOFYEAR(CURDATE()) > DAYOFYEAR(p.doa),1,0) YEAR)  
            BETWEEN DATE_ADD(CURDATE(), INTERVAL 1 DAY) AND DATE_ADD(CURDATE(), INTERVAL 5 DAY) '.$whereStr.'
			order by DAY(p.doa) ASC, p.user_id asc';
		$qry = $db->query($qryStr);
		$res = $qry->fetchAll();
		return $res;
	}
