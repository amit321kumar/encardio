...
<?php     	
    ...
    public static function process_emp_excel($file_name)
    {
        ...
        {    		
            $column_salary_currency = 18;
            $column_salary_type = 19;
            $column_salary = 20; 
            $column_ta = 21; 
            $column_da = 22; 
            $column_hra =23; 
            $column_gratuity = 24; 
            $column_pf =25; 
            //$column_retention_bonus = 26;
            //$column_branch_bonus = 27;
            $column_medical = 28;
            $column_os = 26; 
            $column_cca = 27; 
            $column_pda = 28; 
            ...
            $column_itded = 39; 
            $column_medded = 40;
            //$column_performance_incentive = 29;
            //$column_recreation_allowance = 30;
            //$column_income_tax = 31;
            //$column_superannuation = 32;
            //$column_parking = 33;
            //$column_professional_tax = 34;
            $arrReqHeaders = array(
                ...
                $column_salary_type =>'Pay Frequency',
                $column_salary => 'Salary', 
                $column_ta => 'TA', 
                $column_da => 'DA', 
                $column_hra => 'HRA', 
                $column_gratuity => 'Gratuity', 
                $column_pf => 'PF', 
              //  $column_retention_bonus => ' Retention Bonus',
                //$column_branch_bonus => 'Branch Performance Bonus',
                $column_medical => 'Medical',
                $column_os => 'OS', 
                $column_cca => 'CCA', 
                $column_pda => 'PDA', 
                $column_vpa => 'VPA',  
                $column_arrear => 'Arrear', 
                $column_ot => 'OT', 
                $column_incentive => 'Incentive', 
                $column_bonus => 'Bonus', 
                $column_esi => 'ESI', 
                $column_advance => 'Advance', 
                $column_loan => 'Loan', 
                $column_esiemp => 'ESI Employer',
                $column_itded => 'IT Deductions', 
                $column_medded => 'Medical Deductions',
                //$column_performance_incentive => 'Performance Incentive',
                //$column_recreation_allowance => 'Recreation Allowance',
                //$column_income_tax => 'Income Tax',
                //$column_superannuation => 'Superannuation Allowance',
                //$column_parking => 'Parking',
                //$column_professional_tax = 'Professional Tax'
            ...
            try
            {
                ...
                for($i=2; $i <= $sizeOfWorksheet; $i++ )
                {
                    ...
                    if (!empty($rowData[3]) && strlen($rowData[3]) > 10)
                    {
                        $err_msg = "Employee Id length should be less than or 
                        equal to 10 at row ".$i.".";
                        break;
                    }                    
                    if(empty($rowData[4]))
                    {
					...
					if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_ta])  && !empty($rowData[$column_ta]))
                    {
                        $err_msg = $arrReqHeaders[$column_ta]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_ta]) && $rowData[$column_ta] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_ta]." cannot be zero at row ".$i.".";
                        break;
                    }
					
					if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_da])  && !empty($rowData[$column_da]))
                    {
                        $err_msg = $arrReqHeaders[$column_da]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_da]) && $rowData[$column_da] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_da]." cannot be zero at row ".$i.".";
                        break;
                    }
					
					if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_hra])  && !empty($rowData[$column_hra]))
                    {
                        $err_msg = $arrReqHeaders[$column_hra]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_hra]) && $rowData[$column_hra] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_hra]." cannot be zero at row ".$i.".";
                        break;
                    }
					
					if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_gratuity])  && !empty($rowData[$column_gratuity]))
                    {
                        $err_msg = $arrReqHeaders[$column_gratuity]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_gratuity]) && $rowData[$column_gratuity] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_gratuity]." cannot be zero at row ".$i.".";
                        break;
                    }
					
					if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_pf])  && !empty($rowData[$column_pf]))
                    {
                        $err_msg = $arrReqHeaders[$column_pf]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_pf]) && $rowData[$column_pf] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_pf]." cannot be zero at row ".$i.".";
                        break;
                    }

                /*    if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_retention_bonus])  && !empty($rowData[$column_retention_bonus]))
                    {
                        $err_msg = $arrReqHeaders[$column_retention_bonus]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_retention_bonus]) && $rowData[$column_retention_bonus] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_retention_bonus]." cannot be zero at row ".$i.".";
                        break;
                    }

                    if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_branch_bonus])  && !empty($rowData[$column_branch_bonus]))
                    {
                        $err_msg = $arrReqHeaders[$column_branch_bonus]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_branch_bonus]) && $rowData[$column_branch_bonus] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_branch_bonus]." cannot be zero at row ".$i.".";
                        break;
                    }
                */  
                    if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_medical])  && !empty($rowData[$column_medical]))
                    {
                        $err_msg = $arrReqHeaders[$column_medical]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_medical]) && $rowData[$column_medical] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_medical]." cannot be zero at row ".$i.".";
                        break;
                    }

                    if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_os])  && !empty($rowData[$column_os]))
                    {
                        $err_msg = $arrReqHeaders[$column_os]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_os]) && $rowData[$column_os] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_os]." cannot be zero at row ".$i.".";
                        break;
                    }

                     if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_cca])  && !empty($rowData[$column_cca]))
                    {
                        $err_msg = $arrReqHeaders[$column_cca]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_cca]) && $rowData[$column_cca] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_cca]." cannot be zero at row ".$i.".";
                        break;
                    }

                     if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_pda])  && !empty($rowData[$column_pda]))
                    {
                        $err_msg = $arrReqHeaders[$column_pda]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_pda]) && $rowData[$column_pda] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_pda]." cannot be zero at row ".$i.".";
                        break;
                    }

                     if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_vpa])  && !empty($rowData[$column_vpa]))
                    {
                        $err_msg = $arrReqHeaders[$column_vpa]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_vpa]) && $rowData[$column_vpa] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_vpa]." cannot be zero at row ".$i.".";
                        break;
                    }

                     
                     if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_arrear])  && !empty($rowData[$column_arrear]))
                    {
                        $err_msg = $arrReqHeaders[$column_arrear]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_arrear]) && $rowData[$column_arrear] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_arrear]." cannot be zero at row ".$i.".";
                        break;
                    }

                     if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_ot])  && !empty($rowData[$column_ot]))
                    {
                        $err_msg = $arrReqHeaders[$column_ot]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_ot]) && $rowData[$column_ot] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_ot]." cannot be zero at row ".$i.".";
                        break;
                    }

                     if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_incentive])  && !empty($rowData[$column_incentive]))
                    {
                        $err_msg = $arrReqHeaders[$column_incentive]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_incentive]) && $rowData[$column_incentive] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_incentive]." cannot be zero at row ".$i.".";
                        break;
                    }

                     if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_bonus])  && !empty($rowData[$column_bonus]))
                    {
                        $err_msg = $arrReqHeaders[$column_bonus]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_bonus]) && $rowData[$column_bonus] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_bonus]." cannot be zero at row ".$i.".";
                        break;
                    }

                     if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_esi])  && !empty($rowData[$column_esi]))
                    {
                        $err_msg = $arrReqHeaders[$column_esi]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_esi]) && $rowData[$column_esi] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_esi]." cannot be zero at row ".$i.".";
                        break;
                    }

                     if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_advance])  && !empty($rowData[$column_advance]))
                    {
                        $err_msg = $arrReqHeaders[$column_advance]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_advance]) && $rowData[$column_advance] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_advance]." cannot be zero at row ".$i.".";
                        break;
                    }

                     if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_loan])  && !empty($rowData[$column_loan]))
                    {
                        $err_msg = $arrReqHeaders[$column_loan]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_loan]) && $rowData[$column_loan] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_loan]." cannot be zero at row ".$i.".";
                        break;
                    }

                    if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_esiemp])  && !empty($rowData[$column_esiemp]))
                    {
                        $err_msg = $arrReqHeaders[$column_esiemp]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_esiemp]) && $rowData[$column_esiemp] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_esiemp]." cannot be zero at row ".$i.".";
                        break;
                    }

                    if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_itded])  && !empty($rowData[$column_itded]))
                    {
                        $err_msg = $arrReqHeaders[$column_itded]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_itded]) && $rowData[$column_itded] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_itded]." cannot be zero at row ".$i.".";
                        break;
                    }

                    if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_medded])  && !empty($rowData[$column_medded]))
                    {
                        $err_msg = $arrReqHeaders[$column_medded]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_medded]) && $rowData[$column_medded] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_medded]." cannot be zero at row ".$i.".";
                        break;
                    }

                /*     if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_performance_incentive])  && !empty($rowData[$column_performance_incentive]))
                    {
                        $err_msg = $arrReqHeaders[$column_performance_incentive]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_performance_incentive]) && $rowData[$column_performance_incentive] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_performance_incentive]." cannot be zero at row ".$i.".";
                        break;
                    }

                     if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_recreation_allowance])  && !empty($rowData[$column_recreation_allowance]))
                    {
                        $err_msg = $arrReqHeaders[$column_recreation_allowance]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_recreation_allowance]) && $rowData[$column_recreation_allowance] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_recreation_allowance]." cannot be zero at row ".$i.".";
                        break;
                    }

                     if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_income_tax])  && !empty($rowData[$column_income_tax]))
                    {
                        $err_msg = $arrReqHeaders[$column_income_tax]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_income_tax]) && $rowData[$column_income_tax] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_income_tax]." cannot be zero at row ".$i.".";
                        break;
                    }

                     if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_superannuation])  && !empty($rowData[$column_superannuation]))
                    {
                        $err_msg = $arrReqHeaders[$column_superannuation]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_superannuation]) && $rowData[$column_superannuation] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_superannuation]." cannot be zero at row ".$i.".";
                        break;
                    }

                     if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_parking])  && !empty($rowData[$column_parking]))
                    {
                        $err_msg = $arrReqHeaders[$column_parking]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_parking]) && $rowData[$column_parking] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_parking]." cannot be zero at row ".$i.".";
                        break;
                    }

                     if(!preg_match("/^([0-9]*\.?[0-9]{1,2})$/", $rowData[$column_professional_tax])  && !empty($rowData[$column_professional_tax]))
                    {
                        $err_msg = $arrReqHeaders[$column_professional_tax]." is not a valid format at row ".$i.".";
                        break;
                    }
                    if(!empty($rowData[$column_professional_tax]) && $rowData[$column_professional_tax] == 0)
                    {
                        $err_msg = $arrReqHeaders[$column_professional_tax]." cannot be zero at row ".$i.".";
                        break;
                    }
                */
                    ...
                    for($i=2; $i <= $sizeOfWorksheet; $i++ )
                    {
                        ...
                        if($rowData[$column_salary_currency] !='' || $rowData[$column_salary] != '' || $rowData[$column_salary_type] != '')
                        {
                            $salary_data = array(
								...
								'ta' => !empty($rowData[$column_ta])?sapp_Global::_encrypt($rowData[$column_ta]):null,
					     		'da' => !empty($rowData[$column_da])?sapp_Global::_encrypt($rowData[$column_da]):null,
								'hra' => !empty($rowData[$column_hra])?sapp_Global::_encrypt($rowData[$column_hra]):null,
								'gratuity' => !empty($rowData[$column_gratuity])?sapp_Global::_encrypt($rowData[$column_gratuity]):null,
								'pf' => !empty($rowData[$column_pf])?sapp_Global::_encrypt($rowData[$column_pf]):null,
                      //  'retention_bonus' => !empty($rowData[$column_retention_bonus])?sapp_Global::_encrypt($rowData[$column_retention_bonus]):null,
                        //        'branch_bonus' => !empty($rowData[$column_branch_bonus])?sapp_Global::_encrypt($rowData[$column_branch_bonus]):null,
                                'medical' => !empty($rowData[$column_medical])?sapp_Global::_encrypt($rowData[$column_medical]):null,
                              'os' => !empty($rowData[$column_os])?sapp_Global::_encrypt($rowData[$column_os]):null,
                                'cca' => !empty($rowData[$column_cca])?sapp_Global::_encrypt($rowData[$column_cca]):null,
                                'pda' => !empty($rowData[$column_pda])?sapp_Global::_encrypt($rowData[$column_pda]):null,
                                'vpa' => !empty($rowData[$column_vpa])?sapp_Global::_encrypt($rowData[$column_vpa]):null,
                                'arrear' => !empty($rowData[$column_arrear])?sapp_Global::_encrypt($rowData[$column_arrear]):null,
                                'ot' => !empty($rowData[$column_ot])?sapp_Global::_encrypt($rowData[$column_ot]):null,
                                'incentive' => !empty($rowData[$column_incentive])?sapp_Global::_encrypt($rowData[$column_incentive]):null,
                                'bonus' => !empty($rowData[$column_bonus])?sapp_Global::_encrypt($rowData[$column_bonus]):null,
                                'esi' => !empty($rowData[$column_esi])?sapp_Global::_encrypt($rowData[$column_esi]):null,
                                'advance' => !empty($rowData[$column_advance])?sapp_Global::_encrypt($rowData[$column_advance]):null,
                                'loan' => !empty($rowData[$column_loan])?sapp_Global::_encrypt($rowData[$column_loan]):null,
                                'esiemp`' => !empty($rowData[$column_esiemp])?sapp_Global::_encrypt($rowData[$column_esiemp]):null,
                                'itded' => !empty($rowData[$column_itded])?sapp_Global::_encrypt($rowData[$column_itded]):null,
                                'medded' => !empty($rowData[$column_medded])?sapp_Global::_encrypt($rowData[$column_medded]):null, 
                    /*
                                'performance_incentive' => !empty($rowData[$column_performance_incentive])?sapp_Global::_encrypt($rowData[$column_performance_incentive]):null,
                                'recreation_allowance' => !empty($rowData[$column_recreation_allowance])?sapp_Global::_encrypt($rowData[$column_recreation_allowance]):null,
                                'income_tax' => !empty($rowData[$column_income_tax])?sapp_Global::_encrypt($rowData[$column_income_tax]):null,
                                'superannuation' => !empty($rowData[$column_superannuation])?sapp_Global::_encrypt($rowData[$column_superannuation]):null,
                                'parking' => !empty($rowData[$column_parking])?sapp_Global::_encrypt($rowData[$column_parking]):null,
                                'professional_tax' => !empty($rowData[$column_professional_tax])?sapp_Global::_encrypt($rowData[$column_professional_tax]):null,
                    */
                                'isactive' => 1,
                                'modifiedby'=> $loginUserId,				
                                'modifieddate'=> gmdate("Y-m-d H:i:s"),
                        ...
                        $text = "<div style='padding: 0; text-align: left; font-size:14px; font-family:Arial, Helvetica, sans-serif;'>				
                                        ...
                                        <div style='padding:20px 0 0 0;color:#3b3b3b;'>You have been added to HRMS. The login credentials are:</div>
                        ...
                        $options['header'] = 'Greetings from HRMS!!';
